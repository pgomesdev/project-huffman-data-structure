...
int main()
{
    ...
    if(compactador == 1)
    {
        ...
        for(compactacoes = 0; compactacoes < 3; compactacoes++)
        {
            // String url salva a URL do arquivo texto que será lido
            char url_entrada[1000];
            ...
            if(compactacoes == 0)
            {
                ...
                obter_nome_ext_arq(url_entrada, nome_ex, nome_arquivo);
                ...
                FILE *arq = abrir_Arquivo(url_entrada);
            }
            else
            {
                FILE *arq = abrir_Arquivo(url_saida);
            }
            ...
            if(arq != 0)
            {
                ...
                FILE *novo_arquivo = fopen(url_saida, "w");
    ...
    else if(compactador == 2)
    {
        ...
        for(descompactacoes = 0; descompactacoes < 3; descompactacoes++)
        {
            if(descompactacoes == 0)
            {
                char url_entrada[1000];
                puts("Digite a nome do arquivo a ser DESCOMPACTADO!:");
                scanf("%s",url_entrada);

                FILE *arq_compac = abrir_Arquivo(url_entrada);
            }
            else
            {
                FILE *arq_compac = abrir_Arquivo(url_saida);
            }
            ...
            unsigned short tam_cabecalho = 2 + tam_arvore;
            ...
            if(descompactacoes == 0)
            {
                ...
                tam_cabecalho = (2 + tam_arvore + 1 + tam_extencao + 32 + 1 + tam_nome_arquivo);
            }
            ...
            FILE *arquivo_txt = fopen(url_saida, "w");
